...
extension FileExt on File {
  String relativePath(Directory baseFolder) => p.normalize(p.relative(absolute.path, from: baseFolder.absolute.path));
  ...
  Future<void> copyDirectory(Directory destDir, {bool overwrite = false}) async {
    ...
    for (var srcFile in srcFiles) {
      ...
      if (srcFile.isDirectory()) {
        await srcFile.toDirectory().copyDirectory(destFile.toDirectory(), overwrite: overwrite);
...
extension IterableExt<T> on Iterable<T> {
  ...
  List<T> sortedByButBetter<R extends Comparable<R>>(R? Function(T item) selector, {bool nullsLast = false}) {
  ...
  List<T> sortedByDescending<R extends Comparable<R>>(R? Function(T item) selector, {bool nullsLast = true}) {
    if (isEmpty) return toList();
    return toList()
      ..sort((a, b) {
        final aValue = selector(a);
  ...
  Future<Iterable<T>> whereAsync(FutureOr<bool> Function(T element) test) async {
    var result = <T>[];
